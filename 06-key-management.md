...
- Encryption keys (X25519 key agreement for VC envelope encryption) — deferred to Sub-spec 12. In Phase 1, the reference platform is the sole platform and VC encryption is not required. When multi-platform sync is introduced, X25519 key lifecycle management will be specified alongside the encryption model.
...
- **Federation granularity.** Trust Mark `delegation.authorised_paths` maps issuer DIDs to entity:path permissions. One DID per adapter means fine-grained trust delegation.
...
The platform generates and holds user keys in Cloud KMS. Users don't directly interact with their key material. This is a pragmatic choice — most property professionals don't have digital wallets, and the onboarding friction of self-sovereign keys would be a barrier to adoption.
...
| Property | Value |
|----------|-------|
| **Purpose** | The platform's organisational identity; signs platform-level assertions |
| **DID method** | `did:web:platform.example.com` |
| **Cardinality** | One key |
| **Storage** | Google Cloud KMS (HSM-backed) |
...
**Exception:** If a separate revocation authority is introduced in the future (e.g., a regulatory body that can revoke credentials it didn't issue), it would need its own signing key and Subordinate Entity Statement in the federation. This would be a new issuer with its own key, not a separate "status list key."
...
│ Platform Key     │ did:web      │ KMS (HSM) │ Federation, attestations │
...
```
Google Cloud Organisation: propdata.org.uk
│
...
│   │
│   └── Key Ring: platform
│       └── Key: pdtf-platform-signing-key (Ed25519, HSM)
...
| `platform-service@pdtf-platform-prod.iam` | pdtf-platform-prod | `cloudkms.cryptoKeyVersions.useToSign` | `pdtf-platform-signing-key` |
| `federation-service@pdtf-platform-prod.iam` | pdtf-platform-prod | `cloudkms.cryptoKeyVersions.useToSign` | `pdtf-platform-signing-key` (signs federation Entity Statements + Trust Marks + platform status lists) |
...
└── Platform Key
    ...
    └── pdtf-platform-key
        ...
        ├── Signs: Trust Anchor Entity Statements, Trust Marks, platform attestations
        └── Role: federation Trust Anchor (root — signs all subordinate statements)
```
...
```
Platform Key (did:web:platform.example.com)
    │
    ├── Signs Trust Marks ──→ Adapter DIDs receive pdtf-verified-issuer Trust Marks
    ...
    │       adapter DID + verifying its Trust Mark via federation resolution
    │
    └── User Keys (did:key:z6Mk...)
        └── Sign user-asserted VCs ──→ Verifiable by resolving
            did:key + verifying account-provider Trust Mark via federation resolution
```

**Note:** This is not a certificate chain in the X.509 sense. There's no chain-of-signatures from platform key down to adapter keys. Instead, trust is established by the OpenID Federation: the platform (as Trust Anchor) signs Subordinate Entity Statements and Trust Marks that declare which adapter DIDs are trusted for which credential types. Verification resolves the federation trust chain.
...
5. Issue a `pdtf-verified-issuer` Trust Mark and Subordinate Entity Statement for the adapter DID
   → Populate `delegation.authorised_paths` with the adapter's entity:path permissions
   → Sign with platform (Trust Anchor) key
```
...
The platform key is generated once during initial PDTF 2.0 deployment. The process mirrors adapter key generation but uses the `pdtf-platform-prod` project and publishes the DID document at `https://platform.example.com/.well-known/did.json`.
...
3. Create a "DID rotation" attestation:
   ...
   Signed by the platform key (did:web:platform.example.com)
...
Platform key rotation follows the same pattern as adapter key rotation (§7.1): new key version in KMS, updated DID document at `https://platform.example.com/.well-known/did.json`, old key retained in `verificationMethod`.

**Additional consideration:** Federation Entity Statements and Trust Marks are signed by the platform key. After rotation, they must be re-signed with the new key version and republished. Verifiers should accept federation JWTs signed by any key version listed in the platform (Trust Anchor) DID document / JWKS.
...
| Platform Key | Annually | Immediate | Federation Entity Statements + Trust Marks only |
...
Verification is the counterpart to signing — and it's the more important operation. Anyone can verify a PDTF credential without access to KMS, without being a platform customer, and without any special infrastructure. This is the core value proposition of the VC model.
...
└──────────┬──────────┘
           ...
           ▼
┌─────────────────────┐
│  Federation lookup    │  Is issuer trusted for this entity:path?
└──────────┬──────────┘
           │
           ▼
┌─────────────────────┐
...
async function verifyCredential(
  ...
  // Step 10: Resolve federation trust chain — is this issuer trusted for this credential type?
...
A critical design property: **anyone can verify a PDTF credential without being a platform customer or having access to platform infrastructure.**
...
4. Optionally, access to the Trust Anchor federation endpoints (to check issuer trust) and Status List (to check revocation)
...
Today, the platform generates and manages user keys in Cloud KMS. Users authenticate via Firebase Auth and never directly interact with their cryptographic identity. The credential service signs on their behalf when they make assertions (e.g., completing a TA6 property information form).
...
**Disadvantages:**
- Users must trust the platform not to sign on their behalf without consent
- Not truly self-sovereign — the platform can revoke access to a user's identity
...
When digital identity wallets reach sufficient maturity and adoption in the UK property sector, PDTF 2.0 will support **wallet-bound keys** — where the user's private key lives in their own wallet, not in the platform's KMS.

**How it works:**

```
Today (Custodial):
  User ──→ Platform KMS ──→ did:key:z6Mk...
  (User authenticates, platform signs on their behalf)

Future (Wallet-Bound):
  User ──→ Own Wallet ──→ did:key:z6Mk...
  (User's wallet signs directly, platform never holds the private key)
```
...
Phase 3: Wallet-Preferred (Future)
  ...
  - Existing custodial users can migrate:
    ...
    3. Platform verifies wallet ownership (challenge-response)
...
Phase 4: Wallet-Required (Distant Future)
  ...
  - The platform's role is platform + trusted proxy, not identity provider
```
...
4. WITHIN 1 HOUR: Re-sign all active federation Entity Statements and Trust Marks with the new key
5. WITHIN 4 HOURS: Notify all relying parties that federation metadata
   has been re-signed (old signatures are no longer trusted)
...
| Platform key compromise | Forged Entity Statements / Trust Marks, undermined trust model | HSM, break-glass access, minimal usage, audit logging |
| KMS region outage | Signing unavailable | Cross-region DR, verification unaffected |
| Insider threat (platform operator employee) | Access to signing operations | IAM least-privilege, audit logs, break-glass for admin |
...
□ Issue Trust Marks for adapter DIDs via the Trust Anchor
```
...
| **Trust Anchor** | Root entity of the OpenID Federation — signs Entity Statements and Trust Marks that map issuer DIDs to entity:path trust permissions |
