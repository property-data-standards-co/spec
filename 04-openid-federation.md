# 04 OpenID Federation Trust Architecture
...
The federation is **not** a certificate authority. Issuers manage their own key material (see [Sub-spec 06: Key Management](./06-key-management.md)). The Trust Anchor signs Entity Statements and Trust Marks, not issuer keys.
...
Multiple issuers are permitted for the same entity:path — for example, competing search providers or valuers. The federation does not enforce exclusivity; state assembly (see [Sub-spec 07](./07-state-assembly.md)) merges credentials from multiple issuers for the same path using timestamp-ordered semantics.
