...
- OpenID Federation trust architecture (see [Sub-spec 04: OpenID Federation](./04-openid-federation.md))
...
- Verifiers trace this `did:key` back to a trusted provider via the `account-provider` Trust Mark's `delegation.managed_organisations` document.
...
- The `#sra` and `#companies-house` service endpoints provide machine-readable links to regulatory registrations. The Trust Anchor cross-references these when issuing the Organisation's Trust Mark.
...
1. Consumer discovers adapter DID via OpenID Federation trust resolution
...
**`credentialTypes`** — array of VC type strings this adapter can issue. Enables automated discovery: a platform can enumerate all subordinate entities from the Trust Anchor's `federation_list_endpoint` and determine which adapter to call for a given credential type.
...
- The Trust Mark cross-check (binding DID to regulatory registration) provides a secondary trust anchor
...
| Organisation (`did:web`) | Generate Ed25519 key pair. Create DID document with regulatory metadata. Host at firm's domain. Apply to the Trust Anchor for a `regulated-conveyancer` Trust Mark. |
...
| Trusted Adapter (`did:web`) | Adapter operator generates key pair. Creates DID document with issuance endpoint. Publishes Entity Configuration referencing the Trust Anchor via `authority_hints`; receives a `pdtf-verified-issuer` Trust Mark with `delegation.authorised_paths`. |
...
2. The corresponding Trust Mark is marked revoked via the Trust Anchor's `federation_trust_mark_status_endpoint`
...
|--------|--------|-----------|------------|
| **DNS hijacking** | Attacker serves fraudulent DID document | Low (DNSSEC mitigates) | Federation cross-check: verifier confirms the Subordinate Entity Statement from the Trust Anchor pins the expected JWKS and matches regulatory IDs in the Trust Mark |
| **Domain compromise** | Attacker replaces DID document with their own keys | Low–Medium | Federation cross-check + certificate transparency monitoring + JWKS pinned by Trust Anchor in Subordinate Entity Statement |
...
| **Domain expiry** | Firm lets domain lapse; new owner can serve fraudulent DID doc | Medium (human process risk) | Trust Anchor monitors domain ownership changes; revoke Trust Mark on ownership change |
| **Insider threat** | Firm's IT admin rotates keys and issues fraudulent credentials | Low | Credential verification includes Trust Mark `delegation` check of issuance authorisation |
...
### 9.3 Federation Cross-Check as Defence in Depth
...
3. Resolve issuer trust chain to the Trust Anchor:
   a. Is this DID registered?
   b. Is it authorised for this credential type + entity path?
   c. Does the Trust Mark's delegation / subject metadata match the DID document's regulatory metadata?
   d. Is the Trust Mark still active (status endpoint reports not revoked)?
4. Only if ALL checks pass: credential is trusted
```

This means that even if an attacker compromises a domain and serves a fraudulent DID document, they cannot issue trusted credentials unless they also compromise the Trust Anchor's signing key (which is held in an HSM with separate, independently governed access).
...
2. **Domain verification cadence:** For `did:web` Organisations, how often should the Trust Anchor re-verify that a firm's DID document is still hosted and unchanged before renewing the Subordinate Entity Statement / Trust Mark? (Proposal: daily automated check.)
...
| [04 — OpenID Federation](./04-openid-federation.md) | Federation accreditation of `did:web` entities via Entity Statements and Trust Marks |
